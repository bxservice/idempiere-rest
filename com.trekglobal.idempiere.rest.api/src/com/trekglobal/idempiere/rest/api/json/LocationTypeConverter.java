	...
	private Object toJsonValue(int displayType, String label, Lookup lookup, String refTableName, Object value) {
		if (lookup != null) {
			...
			if (!Util.isEmpty(loc.getAddress1()))
				ref.addProperty(MLocation.COLUMNNAME_Address1, loc.getAddress1());
			if (!Util.isEmpty(loc.getAddress2()))
				ref.addProperty(MLocation.COLUMNNAME_Address2, loc.getAddress2());
			if (!Util.isEmpty(loc.getAddress3()))
				ref.addProperty(MLocation.COLUMNNAME_Address3, loc.getAddress3());
			if (!Util.isEmpty(loc.getAddress4()))
				ref.addProperty(MLocation.COLUMNNAME_Address4, loc.getAddress4());
			if (!Util.isEmpty(loc.getAddress5()))
				ref.addProperty(MLocation.COLUMNNAME_Address5, loc.getAddress5());
			if (!Util.isEmpty(loc.getPostal()))
				ref.addProperty(MLocation.COLUMNNAME_Postal, loc.getAddress5());
			if (!Util.isEmpty(loc.getPostal_Add()))
				ref.addProperty(MLocation.COLUMNNAME_Postal_Add, loc.getPostal_Add());
			
			if (loc.getC_Country_ID() > 0) {
				MCountry c = MCountry.get(loc.getC_Country_ID());
				JsonObject refChild = new JsonObject();
				refChild.addProperty("propertyLabel", Msg.getElement(Env.getCtx(), MCountry.COLUMNNAME_C_Country_ID));
				if (value instanceof Number)
					refChild.addProperty("id", c.get_ID());
				else
					refChild.addProperty("id", value.toString());
				if (!Util.isEmpty(c.getCountryCode(), true)) {
					refChild.addProperty("identifier", c.getCountryCode());
				}							
				refChild.addProperty("model-name", MCountry.Table_Name.toLowerCase());
				ref.add(MCountry.COLUMNNAME_C_Country_ID, refChild);

			}
			
			if (loc.getC_Region_ID() > 0) {
				MRegion r = MRegion.get(loc.getC_Region_ID());
				JsonObject refChild = new JsonObject();
				refChild.addProperty("propertyLabel", Msg.getElement(Env.getCtx(), MRegion.COLUMNNAME_C_Region_ID));
				if (value instanceof Number)
					refChild.addProperty("id", r.get_ID());
				else
					refChild.addProperty("id", value.toString());
				if (!Util.isEmpty(r.getName(), true)) {
					refChild.addProperty("identifier", r.getName());
				}							
				refChild.addProperty("model-name", MRegion.Table_Name.toLowerCase());
				ref.add(MRegion.COLUMNNAME_C_Region_ID, refChild);
			}
			
			if (!Util.isEmpty(loc.getRegionName()))
				ref.addProperty(MLocation.COLUMNNAME_RegionName, loc.getRegionName());
			
			if (loc.getC_City_ID() > 0) {
				MCity c = MCity.get(loc.getC_City_ID());
				JsonObject refChild = new JsonObject();
				refChild.addProperty("propertyLabel", Msg.getElement(Env.getCtx(), MCity.COLUMNNAME_C_City_ID));
				if (value instanceof Number)
					refChild.addProperty("id", c.get_ID());
				else
					refChild.addProperty("id", value.toString());
				if (!Util.isEmpty(c.getName(), true)) {
					refChild.addProperty("identifier", c.getName());
				}							
				refChild.addProperty("model-name", MRegion.Table_Name.toLowerCase());
				ref.add(MCity.COLUMNNAME_C_City_ID, refChild);
			}
			
			if (!Util.isEmpty(loc.getCity()))
				ref.addProperty(MLocation.COLUMNNAME_City, loc.getCity());
			
			if (!Util.isEmpty(loc.getComments()))
				ref.addProperty(MLocation.COLUMNNAME_Comments, loc.getComments());
			
			ref.addProperty(MLocation.COLUMNNAME_IsActive, loc.isActive());
